document.addEventListener("DOMContentLoaded", () => {
  ...
  // Dynamic Bubble Effect for Desktop Sidebar
  navItems.forEach((item) => {
    item.addEventListener("mousemove", (e) => {
      const rect = item.getBoundingClientRect();
      const x = e.clientX - rect.left;
      const y = e.clientY - rect.top;
      item.style.setProperty("--bubble-x", `${x}px`);
      item.style.setProperty("--bubble-y", `${y}px`);
    });
  });
  ...
  postFileInput.onchange = (e) => {
    ...
    reader.onload = (event) => {
      ...
      document.getElementById("modal-me-pfp").src =
        me.profilePicture || "me.png";
  ...
  function updateMeUI() {
    ...
    document.getElementById("prof-fullname").textContent =
      me.fullName || me.username;
    document.getElementById("prof-bio").textContent =
      me.bio || "Digital Creator | Explorer";
    ...
    document.getElementById("total-followers-text").textContent = formatStat(
      me.followersCount,
    );
    renderProfile();
  }
  ...
  async function loadStories() {
    try {
      ...
      const myStoryIndex = activeStories.findIndex(
        (group) => group.user.username === me.username,
      );
      ...
      const dynamicStories = storiesRow.querySelectorAll(
        ".story-circle:not(.add-story)",
      );
    ...
    reader.onload = async (ev) => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          showToast(
            errorData.error || errorData.message || "Failed to upload story",
            "error",
          );
        }
      } catch (err) {
  ...
  const storyProgressBarContainer = document.querySelector(
  ...
  function openStoryViewer() {
    ...
    // Update User Info
    document.getElementById("story-user-pfp").src =
      userGroup.user.profilePicture || "me.png";
    document.getElementById("story-username").textContent =
      userGroup.user.username;
    document.getElementById("story-time").textContent = formatTime(
      segment.createdAt,
    );
  ...
  function updateStoryInteractions() {
    ...
    newLikeBtn.onclick = async (e) => {
      ...
      const res = await fetch(
        `/api/social/story/${storyId}/segment/${segment._id}/like`,
        { method: "POST" },
      );
      const data = await res.json();

      if (data.liked) {
        if (!segment.likes.some((v) => (v._id || v) === myId))
          segment.likes.push(myId);
      } else {
    ...
    if (userGroup.user._id === myId) {
      ...
      document.getElementById("story-viewers-count").textContent =
        `${segment.views.length} viewers`;
  ...
  async function showViewersOverlay(storyId) {
    ...
    try {
      ...
      if (data.viewers.length === 0) {
        listContainer.innerHTML =
          '<p style="text-align:center; color: #999;">No viewers yet</p>';
      }
  ...
  async function deleteStorySegment(segment) {
    ...
    try {
      const res = await fetch(
        `/api/social/story/${storyId}/segment/${segment._id}`,
        { method: "DELETE" },
      );
      const data = await res.json();
  ...
  async function openShareModal(contentObj) {
    ...
    listContainer.innerHTML =
      '<p style="padding: 10px;">Loading connections...</p>';
  ...
  function renderShareList(users) {
    ...
    if (users.length === 0) {
      listContainer.innerHTML =
        '<p style="padding: 20px; text-align: center; color: #8e8e8e;">No people found.</p>';
      return;
    }
    ...
    users.forEach((user) => {
      ...
      div.onclick = () => {
        ...
        document.getElementById("confirm-share-btn").disabled =
  ...
  document.getElementById("share-user-search").oninput = (e) => {
    ...
    // Local Filter
    const filtered = allShareFriends.filter(
      (u) =>
    ...
    if (query.length > 1) {
      shareSearchTimeout = setTimeout(async () => {
        try {
          ...
          const extras = globalUsers.filter(
            (u) => !currentIds.has(u._id) && u._id !== me._id,
          );
  ...
  async function loadConversations() {
    ...
    list.innerHTML =
      '<p style="padding: 20px; color: #8e8e8e;">Loading chats...</p>';
    ...
    try {
      ...
      if (conversations.length === 0) {
        list.innerHTML =
          '<p style="padding: 20px; color: #8e8e8e; text-align: center;">No messages yet. <br> Start a chat!</p>';
      }

      conversations.forEach((conv) => {
        const otherUser = conv.participants.find(
          (p) => p._id !== (me.id || me._id),
        );
        if (!otherUser) return;
  ...
  async function loadMessages(convId) {
    ...
    container.innerHTML =
      '<p style="text-align: center; color: #8e8e8e;">Loading history...</p>';

    try {
      const res = await fetch(
        `/api/messaging/conversation/with/${currentChatRecipient._id}`,
      );
      const data = await res.json();
  ...
  async function loadRequests() {
    try {
      ...
      requestsList.querySelectorAll("button").forEach((btn) => {
        btn.onclick = async () => {
          const action = btn.classList.contains("btn-accept")
            ? "accept"
            : "decline";
          const id = btn.getAttribute("data-id");
          await fetch(`/api/social/requests/${id}/${action}`, {
            method: "POST",
          });
          loadRequests();
  ...
  async function loadFeed() {
    ...
    try {
      ...
      console.error(err);
      feedContainer.innerHTML =
        '<div class="empty-state"><p>Failed to load feed. Please try again.</p></div>';
    }
  }
  ...
  function renderPostsElite(posts) {
    if (posts.length === 0) {
      feedContainer.innerHTML =
        '<div class="empty-state"><p>No posts to show.</p></div>';
      return;
    }
    ...
    posts.forEach((post) => {
      ...
      if (post.isLocked && !post.image) {
        ...
        // Video check
        const isVideo =
          post.image.includes("data:video") || post.image.endsWith(".mp4");
        mediaContent = `
      ...
      downloadIcon.onclick = () =>
        downloadMedia(post.image, `post_${post._id}`);
    });
  }
  ...
  async function toggleLike(postId, icon) {
    ...
    const card =
      icon.closest(".post-card") ||
      document.getElementById("post-detail-modal");
    const likesText =
      card.querySelector(".post-likes-info") ||
      document.getElementById("detail-likes-text");
    ...
    try {
      const res = await fetch(`/api/social/post/${postId}/like`, {
        method: "POST",
      });
      if (!res.ok) {
        const errData = await res.json().catch(() => ({}));
        throw new Error(
          errData.message || errData.error || "Failed to like post",
        );
      }
  ...
  window.unlockPost = async (postId, element) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Replace content
        const isVideo =
          data.image.includes("data:video") || data.image.endsWith(".mp4");
        element.outerHTML = `
    ...
    try {
      ...
      if (currentProfileTab === "posts") {
        ...
        const gridPosts = posts.filter((p) => {
          const isVideo =
            p.image &&
            (p.image.includes("data:video") || p.image.endsWith(".mp4"));
          return !isVideo;
        });
  ...
  async function loadSavedPostsForProfile(container) {
    ...
    container.innerHTML =
      '<p style="padding: 20px;">Loading saved posts...</p>';
    try {
      ...
      console.error(err);
      container.innerHTML =
        '<p style="padding: 20px; color: red;">Failed to load saved posts.</p>';
    }
  }
  ...
  async function openPostDetail(postId) {
    ...
    try {
      ...
      // User Info
      document.getElementById("detail-user-pfp").src =
        post.user.profilePicture || "me.png";
      document.getElementById("detail-username").textContent =
        post.user.username;
      document.getElementById("detail-likes-text").textContent =
        `${formatStat(post.likes.length)} likes`;
      document.getElementById("detail-post-time").textContent = formatTime(
        post.createdAt,
      ).toUpperCase();
      ...
      likeBtn.onclick = async () => {
        ...
        try {
          const lRes = await fetch(`/api/social/post/${post._id}/like`, {
            method: "POST",
          });
          const lData = await lRes.json();
          document.getElementById("detail-likes-text").textContent =
            `${formatStat(lData.likesCount)} likes`;
  ...
  function populateSettings() {
    if (!me) return;
    document.getElementById("edit-pfp-preview").src =
      me.profilePicture || "me.png";
  ...
  document.getElementById("close-settings").onclick = () => {
    ...
    if (!profileView.classList.contains("hidden")) {
      ...
      document
        .querySelector('.nav-item[data-tab="home"]')
        .classList.add("active");
    }
  };
  ...
  async function toggleSavePost(post, icon) {
    ...
    try {
      const res = await fetch(`/api/social/post/${postId}/save`, {
        method: "POST",
      });
  ...
  async function performGlobalSearch(query) {
    ...
    try {
      ...
      if (users.length === 0) {
        resultsContainer.innerHTML =
          '<p style="text-align: center; padding: 40px; color: #8e8e8e;">No people found.</p>';
        return;
      }
  ...
  function renderUserSearchCard(user, container) {
    ...
    followBtn.onclick = async () => {
      const res = await fetch(`/api/social/follow/${user._id}`, {
        method: "POST",
      });
      if (res.ok) {
        const data = await res.json();
  ...
  function initActivityView(targetTab = "favorites") {
    ...
    tabs.forEach((tab) => {
      tab.onclick = () => {
        ...
        document
          .querySelectorAll(".act-content-block")
          .forEach((b) => b.classList.add("hidden"));
        document
          .getElementById(`activity-content-${target}`)
    ...
    const targetBtn = Array.from(tabs).find(
      (t) => t.getAttribute("data-act-tab") === targetTab,
    );
  ...
  async function loadSavedPosts() {
    ...
    container.innerHTML =
      '<p style="padding: 20px;">Loading your favorites...</p>';
    ...
    try {
      ...
      if (posts.length === 0) {
        container.innerHTML =
          '<p style="padding: 40px; grid-column: 1/4; text-align: center; color: #8e8e8e;">No saved posts yet.</p>';
        return;
      }
  ...
  async function loadActivityFeed() {
    ...
    try {
      ...
      if (activities.length === 0) {
        container.innerHTML =
          '<p style="padding: 40px; text-align: center; color: #8e8e8e;">No recent activity.</p>';
        return;
      }
  ...
  function showPostOptions(post) {
    ...
    globalDeleteBtn.onclick = async () => {
      ...
      try {
        const res = await fetch(`/api/social/post/${post._id}`, {
          method: "DELETE",
        });
        if (res.ok) {
  ...
  btnEditProfiles.forEach((btn) => {
    btn.onclick = () => {
      setModal.classList.remove("hidden");
      const editTab = Array.from(document.querySelectorAll(".set-tab")).find(
        (t) => t.getAttribute("data-set-tab") === "edit-profile",
      );
      if (editTab) editTab.click();
    };
  });

  const btnViewArchives = document.querySelectorAll(
    ".btn-prof-action.secondary",
  );
  btnViewArchives.forEach((btn) => {
    btn.onclick = () =>
      showToast("View archive is currently unavailable.", "normal");
  });
});
